...
const BookingModal = ({ show, onClose, venue, bookingType, onSetBookingType, user }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (bookingType === "direct") {
        ...
        alert('🔍 تم إرسال طلب المعاينة بنجاح! طلبك قيد المعالجة وسنتواصل معك قريباً.');
      }
      ...
      >
        {!bookingType ? (
          <BookingTypeSelection 
            venue={venue}
            onSetBookingType={onSetBookingType}
            onClose={onClose}
          />
        ) : (
          <BookingForm 
...
// Sub-components for BookingModal
const BookingTypeSelection = ({ venue, onSetBookingType, onClose }) => (
  <div className="p-6">
    ...
    <div className="space-y-4">
      <div 
        onClick={() => onSetBookingType("direct")}
      ...
      <div 
        onClick={() => onSetBookingType("inspection")}
    ...
    <div className="mt-6 p-4 bg-gray-50 rounded-lg">
      ...
      <p className="text-sm text-gray-600">{venue.city}، {venue.governorate}</p>
      <p className="text-sm font-semibold text-purple-600">
        {parseInt(venue.price)?.toLocaleString()} جنيه
      </p>
      {venue.originalPrice && (
        <p className="text-sm text-red-500 line-through">
          كان {parseInt(venue.originalPrice)?.toLocaleString()} جنيه
        </p>
      )}
    </div>
  </div>
...
const BookingForm = ({ 
  venue, bookingType, formData, setFormData, bookingLoading, 
  bookingSuccess, smsStatus, onSetBookingType, onSubmit, user 
}) => (
  <div className="p-6">
    <div className="flex justify-between items-center mb-6">
      <div>
    ...
    {bookingSuccess ? (
      <BookingSuccess bookingType={bookingType} />
    ) : (
...
const DirectBookingForm = ({ formData, setFormData, venue }) => (
  <>
    <div>
...
const InspectionBookingForm = ({ formData, setFormData }) => (
  <>
...
const BookingSuccess = ({ bookingType }) => (
